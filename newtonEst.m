function [output] = newtonEst(signal,r,thresh)
%newtonEst 
% 
%This function can be used to compress an audio file and will
%output a compressed audio file with '_sn.wav' attached to the file name
% 
%   signal needs to be a string location of the audio file to use
%   
%   for example, signal = ('audioFile.wav')
% 
...
%   outfile = newtonEst('male_5sec.wav',3,0.4);
% 

% Envelope Single File Test

    [v_data, fs] = audioread(signal); %Read external wav file
    ...
    dotLocations = find(signal == '.')
    if isempty(dotLocations)
        % No dots at all found so just take entire name.
        nameBeforeFirstDot = signal
    else
        % Take up to , but not including, the first dot.
        nameBeforeFirstDot = signal(1:dotLocations(1)-1)
    end
    
    outName = strcat(nameBeforeFirstDot, '_sn.wav');
